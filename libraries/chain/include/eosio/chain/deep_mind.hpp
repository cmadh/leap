...
{
public:
    virtual ~deep_mind_handler_base(){}
    ...
    virtual void update_config(deep_mind_config config);

    virtual void update_logger(const std::string& logger_name);
    enum class operation_qualifier { none, modify, push };

    virtual void on_startup(chainbase::database& db, uint32_t head_block_num);
    virtual void on_start_block(uint32_t block_num);
    virtual void on_accepted_block(const std::shared_ptr<block_state>& bsp);
    virtual void on_switch_forks(const block_id_type& old_head, const block_id_type& new_head);
    virtual void on_onerror(const signed_transaction& etrx);
    virtual void on_onblock(const signed_transaction& trx);
    virtual void on_start_transaction();
    virtual void on_end_transaction();
    virtual void on_applied_transaction(uint32_t block_num, const std::shared_ptr<transaction_trace>& trace);
    virtual void on_add_ram_correction(const account_ram_correction_object& rco, uint64_t delta);
    virtual void on_preactivate_feature(const protocol_feature& feature);
    virtual void on_activate_feature(const protocol_feature& feature);
    virtual void on_input_action();
    virtual void on_end_action();
    virtual void on_require_recipient();
    virtual void on_send_inline();
    virtual void on_send_context_free_inline();
    virtual void on_cancel_deferred(operation_qualifier qual, const generated_transaction_object& gto);
    virtual void on_send_deferred(operation_qualifier qual, const generated_transaction_object& gto);
    virtual void on_create_deferred(operation_qualifier qual, const generated_transaction_object& gto, const packed_transaction& packed_trx);
    virtual void on_fail_deferred();
    virtual void on_create_table(const table_id_object& tid);
    virtual void on_remove_table(const table_id_object& tid);
    virtual void on_db_store_i64(const table_id_object& tid, const key_value_object& kvo);
    virtual void on_db_update_i64(const table_id_object& tid, const key_value_object& kvo, account_name payer, const char* buffer, std::size_t buffer_size);
    virtual void on_db_remove_i64(const table_id_object& tid, const key_value_object& kvo);
    virtual void on_init_resource_limits(const resource_limits::resource_limits_config_object& config, const resource_limits::resource_limits_state_object& state);
    virtual void on_update_resource_limits_config(const resource_limits::resource_limits_config_object& config);
    virtual void on_update_resource_limits_state(const resource_limits::resource_limits_state_object& state);
    virtual void on_newaccount_resource_limits(const resource_limits::resource_limits_object& limits, const resource_limits::resource_usage_object& usage);
    virtual void on_update_account_usage(const resource_limits::resource_usage_object& usage);
    virtual void on_set_account_limits(const resource_limits::resource_limits_object& limits);
    // The trace is consumed by the next ram_event or ram_correction
    virtual void on_ram_trace(std::string&& event_id, const char* family, const char* operation, const char* legacy_tag);
    virtual void on_ram_event(account_name account, uint64_t new_usage, int64_t delta);
    virtual void on_create_permission(const permission_object& p);
    virtual void on_modify_permission(const permission_object& old_permission, const permission_object& new_permission);
    virtual void on_remove_permission(const permission_object& permission);
private:
